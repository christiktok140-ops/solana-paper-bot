...
// ── Access middleware — admin always bypasses ─────────────────
async function checkAccess(ctx, next) {
    ...
    if (['/start', '/pay'].includes(text)) return next();

    // Check subscription/vip
    ...
    await ctx.reply(
        ...
        `─────────────────\n` +
        `⚠️ *After sending SOL:*\n` +
        `Contact @Supryme_loves_memecoins with proof\n\n` +
        `─────────────────`,
        { parse_mode: 'Markdown' }
    );
}
...
bot.command('pay', async (ctx) => {
    ...
    await ctx.reply(
        ...
        `─────────────────\n` +
        `💡 *Once verified, access granted within 24 hours*`,
        { parse_mode: 'Markdown' }
    );
});
...
bot.command('grantaccess', async (ctx) => {
    ...
    await ctx.reply(`✅ Granted ${months} month(s) to ${targetId}\nExpires: ${expiry.toLocaleDateString()}`);
});
...
bot.command('test', async (ctx) => {
    ...
    try {
        ...
        if (result) { await ctx.reply(`✅ API Working! SOL Price: $${result.priceUsd}`); }
