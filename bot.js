...
// Faster price cache - 3 seconds TTL
...
async function getTokenData(mintAddress, forceRefresh = false) {
    // Check cache first (unless force refresh)
    if (!forceRefresh && priceCache.has(mintAddress)) {
        const cached = priceCache.get(mintAddress);
        if (Date.now() - cached.timestamp < CACHE_TTL) {
            console.log(`🔄 Using cached price for ${mintAddress.slice(0, 8)}...`);
            return cached.data;
        }
    }
    
    // Avoid duplicate concurrent requests for the same token
    if (pendingRequests.has(mintAddress)) {
        console.log(`⏳ Waiting for pending request for ${mintAddress.slice(0, 8)}...`);
        return await pendingRequests.get(mintAddress);
    }
    
    const promise = (async () => {
        try {
            const response = await axios.get(`https://api.dexscreener.com/latest/dex/tokens/${mintAddress}`, { timeout: 6000 });
            ...
            console.log(`Error fetching ${mintAddress}:`, error.message);
            return null;
        } finally {
...
bot.command('test', async (ctx) => {
    const result = await getTokenData('So11111111111111111111111111111111111111112', true);
...
bot.on('text', async (ctx) => {
    ...
    const tokenData = await getTokenData(text, true);
...
for (const percent of [25, 50, 75, 100]) {
    bot.action(`sell_${percent}`, async (ctx) => {
        ...
        const fresh = await getTokenData(mint, true); // Force refresh
        if (fresh) await updateTokenCard(ctx, fresh, userId, messageId);
    });
}
