...
const TRIAL_DAYS = 0;
const FLAT_FEE = 2.50; // $2.50 flat fee for buy and sell
...
db.exec(`
    CREATE TABLE IF NOT EXISTS portfolios (
        ...
        virtualUsd REAL NOT NULL DEFAULT 100.0,
...
async function initializeUser(userId) {
    ...
    if (!userPortfolios.has(userIdStr)) {
        ...
        userPortfolios.set(userIdStr, {
            virtualUsd: saved?.virtualUsd ?? 100.0,
...
async function paperBuy(userId, mint, tokenData, amountUsd) {
    ...
    // ✅ CHANGED: Flat fee $2.50
    const buyFee = FLAT_FEE;
    const totalCost = amountUsd + buyFee;
    if (portfolio.virtualUsd < totalCost) return { success: false, error: `Need $${totalCost.toFixed(2)} inc. $${FLAT_FEE} fee` };
...
bot.command('refill', async (ctx) => {
    const userId = String(ctx.from.id);
    userPortfolios.set(userId, { virtualUsd: 100.0, holdings: new Map(), trades: [], cards: new Map(), currentMint: null, pendingAction: null, currentTokenData: null, lastMessageId: null, lastMessageHasPhoto: false, pendingMessageId: null });
    saveUser(userId, userPortfolios.get(userId));
    await ctx.reply('✅ Portfolio refilled! You have $100.00');
});
...
// ✅ UPDATED: History with pagination (shows 10 trades per page)
let historyPages = {};
...
bot.command('history', async (ctx) => {
    ...
    for (const trade of sells) { 
        if (trade.profitUsd > 0) wins++; 
        else if (trade.profitUsd < 0) losses++; 
        totalPnl += trade.profitUsd; 
        totalInvested += trade.amountUsd; 
    }
    ...
    const divider = `─────────────────`;
    const pageSize = 10;
    const totalPages = Math.ceil(sells.length / pageSize);
    
    // Store pagination data for this user
    if (!historyPages[userId]) historyPages[userId] = { page: 0 };
    historyPages[userId].page = 0;
    historyPages[userId].sells = sells;
    historyPages[userId].totalPages = totalPages;
    
    await sendHistoryPage(ctx, userId, 0);
});

async function sendHistoryPage(ctx, userId, page) {
    const data = historyPages[userId];
    if (!data) return;
    ...
    const end = Math.min(start + pageSize, sells.length);
    const pageSells = sells.slice(start, end);
    ...
    pageSells.forEach((trade, i) => {
        ...
        message += `💸 Fee:         *$${(trade.fee || 0).toFixed(2)}* _(Flat $${FLAT_FEE})_\n`;
    ...
    // Pagination buttons
    const buttons = [];
    if (page > 0) buttons.push(Markup.button.callback('◀️ Prev', `history_page_${page - 1}`));
    buttons.push(Markup.button.callback(`📄 ${page + 1}/${totalPages}`, 'history_current'));
    if (page < totalPages - 1) buttons.push(Markup.button.callback('Next ▶️', `history_page_${page + 1}`));
...
bot.action(/history_page_(\d+)/, async (ctx) => {
    ...
    await sendHistoryPage(ctx, userId, page);
});

bot.action('history_current', async (ctx) => {
...
bot.on('text', async (ctx) => {
    ...
    if (p.pendingAction === 'buy_x') {
        ...
        await ctx.reply(
            ...
            `💸 Fee:       *$${result.fee.toFixed(2)}* _(Flat $${FLAT_FEE})_\n` +
