...
async function saveUser(userId, portfolio) {
    try {
        ...
        await portfoliosCollection.updateOne(
            ...
            { $set: { virtualUsd: portfolio.virtualUsd, holdings: holdingsObj, trades: portfolio.trades.slice(0, 500), updatedAt: new Date() } },
            { upsert: true }
        );
...
async function paperBuy(userId, mint, tokenData, amountUsd) {
    ...
    if (portfolio.virtualUsd < totalCost) return { success: false, error: `You only have $${portfolio.virtualUsd.toFixed(2)} (need $${totalCost.toFixed(2)} inc. 1% fee)` };
...
bot.start(async (ctx) => {
    ...
    let accessLine = '';
    if (userId === ADMIN_ID || vip) {
        accessLine = `✅ *Access: Lifetime Free*\n`;
    } else if (user?.subscribedUntil && new Date(user.subscribedUntil) > new Date()) {
        const expiry = new Date(user.subscribedUntil).toLocaleDateString();
        accessLine = `✅ *Subscribed until: ${expiry}*\n`;
    } else {
        accessLine = `⚠️ *No active subscription — /pay to subscribe*\n`;
    }
    const divider = `─────────────────`;
...
for (const percent of [25, 50, 75, 100]) {
    bot.action(`sell_${percent}`, async (ctx) => {
        ...
        const card = pendingMsgId ? portfolio.cards.get(pendingMsgId) : null;
        await ctx.answerCbQuery();
        let sellTokenData = portfolio.currentTokenData;
        const fresh = await getTokenData(portfolio.currentMint);
        if (fresh) { sellTokenData = fresh; portfolio.currentTokenData = fresh; if (card) card.tokenData = fresh; }
        ...
        await updateTokenCard(ctx, sellTokenData, portfolio);
    });
}
