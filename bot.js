...
// Cache with shorter TTL and force refresh option
...
// FIXED: Refresh button now forces a fresh API call
bot.action('refresh', async (ctx) => {
    ...
    const freshData = await getTokenData(mint, true); // force refresh
    if (!freshData) return ctx.answerCbQuery('❌ Failed to fetch price');
    await updateTokenCard(ctx, freshData, userId, messageId);
});

bot.launch();
console.log('🤖 Bot is running with JSON storage and fixed refresh!');
